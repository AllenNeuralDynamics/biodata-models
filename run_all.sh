for file in src/biodata_models/_generators/templates/*.txt; do
    ...
    if [ "$type_name" == "harp_types" ]; then
        python src/biodata_models/_generators/update_harp_types.py
    fi
    
    # Call the Python script with the --type parameter
    python src/biodata_models/_generators/generator.py --type "$type_name"
done